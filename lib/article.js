import { getPostBySlug, getCategories, getTags, getRecentPosts } from './posts.js';
import { getMagazineByPostId } from './magazines.js';
...
export async function getArticleData(slug) {
  const [post, categories, tags, recent] = await Promise.all([
    getPostBySlug(slug),
    getCategories(),
    getTags(),
  ...
  const mustReadPosts = recent.filter((p) => p.slug !== slug).slice(0, 2);

  // Backlink to the magazine this post is the cover story of (if any).
  const relatedMagazine = await getMagazineByPostId(post.uuid);

  let rankingEntries = [];
  if (post.post_type === 'ranking' && post.uuid) {
    const { data } = await db
