...
const SELECT = `
  wp_id, slug, title, subtitle, content, excerpt, featured_image, published_at, updated_at,
...
export async function getPostBySlug(slug) {
  const { data, error } = await published(db.from('posts').select(SELECT).eq('slug', slug)).maybeSingle();
  if (error) throw error;
