...
export async function savePost(input, id = null) {
  ...
  const row = {
    ...
    send_to_newsletter: !!input.send_to_newsletter,
  ...
  if (Array.isArray(input.social_promotions)) {
    await db.from('social_promotions').delete().eq('post_id', postId);
    const rows = input.social_promotions
      .filter((s) => s && s.channel)
      .map((s) => ({
        post_id: postId,
        channel: s.channel,
    ...
    if (rows.length) await db.from('social_promotions').insert(rows);
  }
